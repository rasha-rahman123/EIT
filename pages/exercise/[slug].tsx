...
export const ExercisePage : React.FC = ({}) => {
  ...
  const [whyInfo, setWhyInfo] = useState<string>();
  const [loading, setLoading] = useState<boolean>(true);
  const [liked, setLiked] = useState<boolean>();
  const router = useRouter()
  const {query} : any = router
  useEffect(() => {
    router && (setTimeout(() => setQueries(query),1000));
    const j = (queries) && args.filter((x) => x.slug === query.slug);
    queries && (setTaskInfo(j[0].taskInfo));
    queries && (setTimeInfo(j[0].time + " min"));
    queries && (setWhyInfo(j[0].whyInfo));
    (queries) && taskInfo && timeInfo && whyInfo && setLoading(false);
    ...
    >
      {loading ? (
        <>
          ...
          >
            <Loading width={'168px'} height={'189px'} rows={1} />
          </Box>
          <Box sx={{ width: "100%", textAlign: "center" }}>
            <Loading width={'314px'} height={'20px'} rows={1} />
          </Box>
          <Loading width="90vw" height={'20px'} rows={4} />
          ...
          >
            <Loading width={'314px'} height={'70px'} rows={1} />
          </Box>
        </>
      ) : (
        <>
          ...
          {query && query.slug && args ? (
            ...
            <Loading width={'100px'} height={'100px'} rows={1} />
          )}
          <Box
            as="a"
            href={`/exercise/active/${query && query.slug}`}
            sx={{
              width: "100%",
              mt: 2,
              display: "flex",
