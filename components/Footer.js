...
export const Footer = ({}) => {
    ...
    const [bigFooter, setBigFooter] = useState();
      ...
      >
          <Text onClick={() => setBigFooter(false)} sx={{position: 'absolute', bottom: 20, right: 20, fontSize: 3, fontWeight: 800, opacity: 0.6, cursor: 'pointer', ":hover":{opacity: 1}}}>X</Text>
            <Box
          sx={{
            display: "flex",
            flexDirection: "row",
        ...
        >
          <Text sx={{cursor: 'pointer',}}onClick={() => setAboutModal(true)}>About</Text>{" "}
          <Modal
            isOpen={aboutModal}
          ...
          >
            ...
            <Text fontSize={2} fontWeight={800} color={"brayyy"}>
              ...
              profesional academy and I am trying to grow so that I can!
              These are just methods I believe would work based on hundreds
              of studies that I, as psychology student and on freetime, have
          ...
          <Text  sx={{cursor: 'pointer',}} onClick={() => setContactModal(true)}>Contact</Text>
          <Modal
            isOpen={contactModal}
          ...
          If you have any comments, questions, or concerns
            ...
            {formSent ? <Text>Thank you!</Text> : <Box as="form" onSubmit={(e) => {
              e.preventDefault();
            ...
            <Label>MESSAGE</Label>
            <Textarea value={contactMsg} onChange={e => setContactMsg(e.target.value)} mb={2} />
            <ReCAPTCHA sitekey={'6LdD9CYaAAAAAKJ_MscBpjZfnGbkzJ_-h-4ZcThX'} onExpired={() => setVerified(false)} onChange={() => setVerified(true)} />
            {verified ? <Button sx={{bg: 'brayyy', color: 'white', mt: 2, alignSelf: 'center'}} type="submit">Send</Button> : <Label mt={2}>Please Complete Captcha</Label>}
            </Box>}
          </Modal>
          <Text  sx={{cursor: 'pointer',}} onClick={() => setTOSModal(true)}>Terms of Service</Text>
          <Modal
            isOpen={tosModal}
            onRequestClose={() => setTOSModal(false)}
            style={customStyles}
            contentLabel="TOS Page"
          >IN PROGRESS (BETA)</Modal>
        </Box>
      </Text> : <Text onClick={() => setBigFooter(true)} sx={{
          position: "absolute",
          py: 2,
          bottom: 0,
          fontSize: 1,
